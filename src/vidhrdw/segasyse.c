...
VIDEO_UPDATE( megrescu )
{
	...
	for (i = 0;i < 192;i++)
		draw_scanline8(bitmap,0,i,256,&cache_bitmap[i * (16+256+16) +16],&Machine->pens[segasyse_palettebase],-1); /* otherwise broken sprites */
}
...
void segae_drawspriteline(UINT8 *dest, UINT8 chip, UINT8 line)
{
	...
	if (segae_vdp_regs[chip][1] & 0x1) {
		ui_popup("double-size spr. not supported. ");
		return;
	}
