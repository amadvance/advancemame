...
static void draw_sprites_cbombers_16x16(mame_bitmap *bitmap,const rectangle *cliprect,const int *primasks,int x_offs,int y_offs)
{
	UINT16 *spritemap = (UINT16 *)memory_region(REGION_USER1);
...
/* The "PIV" chip seems to be a renamed TC0100SCN. It has a
   ...
   serve some blending pupose ? */
	...
	draw_crosshair( bitmap, (255 - readinputport(3)) * 1.255, readinputport(4) + 9, cliprect, 0 );
	draw_crosshair( bitmap, (255 - readinputport(5)) * 1.255, readinputport(6) + 9, cliprect, 1 );
