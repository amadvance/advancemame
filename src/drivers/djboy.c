...
static WRITE8_HANDLER ( djboy_spriteram_w ) // pandora
{
	offset = BITSWAP16(offset,  15,14,13,12, 11,   7,6,5,4,3,2,1,0,   10,9,8  );
	spriteram[offset] = data;
}

static READ8_HANDLER( djboy_spriteram_r ) // pandora
{
	offset = BITSWAP16(offset,  15,14,13,12, 11,  7,6,5,4,3,2,1,0,  10,9,8  );
...
static ADDRESS_MAP_START( cpu1_readmem, ADDRESS_SPACE_PROGRAM, 8 )
	...
	AM_RANGE(0xb000, 0xbfff) AM_READ(djboy_spriteram_r)
...
static ADDRESS_MAP_START( cpu1_writemem, ADDRESS_SPACE_PROGRAM, 8 )
	...
	AM_RANGE(0xb000, 0xbfff) AM_WRITE(djboy_spriteram_w) AM_BASE(&spriteram) AM_SIZE(&spriteram_size)
...
static ADDRESS_MAP_START( cpu2_readmem, ADDRESS_SPACE_PROGRAM, 8 )
	...
	AM_RANGE(0xd400, 0xd8ff) AM_READ(MRA8_RAM) /* workram */
...
static ADDRESS_MAP_START( writeport2, ADDRESS_SPACE_IO, 8 )
	...
	AM_RANGE(0x04, 0x04) AM_WRITE(beast_data_w)
...
GAME( 1989, djboy,  0,      djboy,   djboy, djboy,  ROT0, "Kaneko (American Sammy license)", "DJ Boy", 0 ) // Sammy & Williams logos in FG ROM
