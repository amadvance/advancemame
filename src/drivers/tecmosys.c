...
static mame_bitmap *tmp_tilemap_composebitmap;
static mame_bitmap *tmp_tilemap_renderbitmap;
...
static ADDRESS_MAP_START( readmem, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x210000, 0x210001) AM_READ(MRA16_NOP) // single byte overflow on stack defined as 0x210000
	AM_RANGE(0x300000, 0x3013ff) AM_READ(MRA16_RAM) // bg0 ram
	AM_RANGE(0x400000, 0x4013ff) AM_READ(MRA16_RAM) // bg1 ram
	AM_RANGE(0x500000, 0x5013ff) AM_READ(MRA16_RAM) // bg2 ram
	AM_RANGE(0x700000, 0x703fff) AM_READ(MRA16_RAM) // fix ram   (all these names from test screen)
	AM_RANGE(0x800000, 0x80ffff) AM_READ(MRA16_RAM) // obj ram
	AM_RANGE(0x880000, 0x88000b) AM_READ(unk880000_r)
	AM_RANGE(0x900000, 0x907fff) AM_READ(MRA16_RAM) // obj pal
	AM_RANGE(0x980000, 0x9807ff) AM_READ(MRA16_RAM) // bg pal
	AM_RANGE(0x980800, 0x980fff) AM_READ(MRA16_RAM) // fix pal
...
static WRITE16_HANDLER( eeprom_w )
{
	if ( ACCESSING_MSB )
	{
		EEPROM_write_bit(data & 0x0800 >> 8);
...
static ADDRESS_MAP_START( writemem, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x800000, 0x80ffff) AM_WRITE(MWA16_RAM) AM_BASE(&tecmosys_spriteram) // obj ram
	...
	AM_RANGE(0xa80000, 0xa80005) AM_WRITE(MWA16_RAM	) AM_BASE(&tecmosys_a80000regs)	// a80000-3 scroll? a80004 inverted ? 3 : 0
	AM_RANGE(0xb00000, 0xb00005) AM_WRITE(MWA16_RAM	) AM_BASE(&tecmosys_b00000regs)	// b00000-3 scrool?, b00004 inverted ? 3 : 0
	AM_RANGE(0xb80000, 0xb80001) AM_WRITE(prot_status_w)
	AM_RANGE(0xc00000, 0xc00005) AM_WRITE(MWA16_RAM	) AM_BASE(&tecmosys_c00000regs)	// c00000-3 scroll? c00004 inverted ? 13 : 10
	AM_RANGE(0xc80000, 0xc80005) AM_WRITE(MWA16_RAM	) AM_BASE(&tecmosys_c80000regs)	// c80000-3 scrool? c80004 inverted ? 3 : 0
...
static const gfx_decode tecmosys_gfxdecodeinfo[] =
{
...
static ADDRESS_MAP_START( readport, ADDRESS_SPACE_IO, 8 )
	...
	AM_RANGE(0x10, 0x10) AM_READ(OKIM6295_status_0_r)
...
static void tecmosys_render_sprites_to_bitmap(mame_bitmap *bitmap, UINT16 extrax, UINT16 extray )
{
...
void tecmosys_do_final_mix(mame_bitmap *bitmap)
{
...
static MACHINE_DRIVER_START( deroon )
	...
	MDRV_SOUND_ADD(OKIM6295, 16000000/8/165)
...
GAME( 1995, deroon2,     deroon,   deroon, deroon, deroon,     ROT0, "Tecmo", "Deroon DeroDero / Tecmo Stackers", 0 )
GAME( 1996, tkdensho,    0,        deroon, deroon, tkdensho,   ROT0, "Tecmo", "Toukidenshou - Angel Eyes (VER. 960614)", 0 )
GAME( 1996, tkdensha,    tkdensho, deroon, deroon, tkdensha,   ROT0, "Tecmo", "Toukidenshou - Angel Eyes (VER. 960427)", 0 )
