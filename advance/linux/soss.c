...
video_error sound_oss_init(int sound_id, unsigned* rate, video_bool stereo_flag, double buffer_time)
{
	...
	os_log(("sound:oss: sound_oss_init(id:%d,rate:%d,stereo:%d,buffer_time:%g)\n",sound_id,*rate,stereo_flag,buffer_time));
	...
	if (!oss_state.handle) {
		os_log(("sound:oss: open(/dev/dep,O_WRONLY | O_NONBLOCK,0) failed\n"));
		goto err;
	}

	i = AFMT_S16_LE;
	if (ioctl(oss_state.handle, SNDCTL_DSP_SETFMT, &i) < 0) {
		os_log(("sound:oss: ioctl(SNDCTL_DSP_SETFMT, AFMT_S16_LE) failed\n"));
		goto err_close;
	}
	if (i != AFMT_S16_LE) {
		os_log(("sound:oss: ioctl(SNDCTL_DSP_SETFMT, AFMT_S16_LE) return a different format\n"));
		goto err_close;
	}
	...
	if (ioctl(oss_state.handle, SNDCTL_DSP_CHANNELS, &i) < 0) {
		os_log(("sound:oss: ioctl(SNDCTL_DSP_CHANNELS, %d) failed\n",i));
		goto err_close;
	}

	i = *rate;
	if (ioctl(oss_state.handle, SNDCTL_DSP_SPEED, &i) < 0) {
		os_log(("sound:oss: ioctl(SNDCTL_DSP_SPEED, %d) failed\n",i));
		goto err_close;
	}
	oss_state.rate = i;
	...
	if (ioctl(oss_state.handle, SNDCTL_DSP_SETFRAGMENT, &i) < 0) {
		os_log(("sound:oss: ioctl(SNDCTL_DSP_SETFRAGMENT,%d:%d) failed\n",i >> 16,i & 0xFFFF));
		goto err_close;
	}

	if (ioctl(oss_state.handle, SNDCTL_DSP_GETOSPACE, &info) < 0) {
		os_log(("sound:oss: ioctl(SNDCTL_DSP_GETOSPACE) failed\n"));
		goto err_close;
	}

	os_log(("sound:oss: sound getospace() = fragsize %d, fragtotal %d, freebytes %d\n",(int)info.fragsize,(int)info.fragstotal,(int)info.bytes));
...
void sound_oss_done(void) {
	os_log(("sound:oss: sound_oss_done()\n"));
...
void sound_oss_stop(void) {
	os_log(("sound:oss: sound_oss_stop()\n"));
	/* TODO OSS sound stop */
}
...
unsigned sound_oss_buffered(void) {
	...
	os_log_debug(("sound:oss: getospace() = fragsize %d, fragtotal %d, freebytes %d\n",(int)info.fragsize,(int)info.fragstotal,(int)info.bytes));
...
void sound_oss_volume(double volume) {
	os_log(("sound:oss: sound_oss_volume(volume:%g)\n",(double)volume));
...
void sound_oss_play(const short* sample_map, unsigned sample_count) {
	...
	os_log_debug(("sound:oss: sound_oss_play(count:%d)\n",sample_count));
	...
	if (sample_count) {
		...
		if (r != sample_count * oss_state.sample_length) {
			os_log(("sound:oss: sound WRITE ERROR %d\n",r));
		}
	}
}
...
video_error sound_oss_start(double silence_time) {
	...
	os_log(("sound:oss: sound_oss_start(silence_time:%g)\n",silence_time));
