...
static unsigned sound_allegro_current(void) {
	int play_pos;
	play_pos = voice_get_position(allegro_state.voice);
	return play_pos;
}

unsigned sound_allegro_buffered(void) {
	unsigned play_pos = sound_allegro_current();
	unsigned missing;
...
static video_bool sound_allegro_overflow(unsigned pos, unsigned length) {
	unsigned play_pos;
...
unsigned sound_allegro_flags(void) {
	return 0;
}

video_error sound_allegro_load(struct conf_context* context) {
	return 0;
}
...
sound_driver sound_allegro_driver = {
	"allegro",
	DEVICE,
	sound_allegro_load,
	sound_allegro_reg,
	...
	sound_allegro_flags,
	sound_allegro_play,
	sound_allegro_buffered,
	sound_allegro_start,
	sound_allegro_stop,
	sound_allegro_volume
};
