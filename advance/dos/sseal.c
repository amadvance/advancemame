...
video_error sound_seal_init(int device_id, unsigned* rate, video_bool stereo_flag, double buffer_time) {
	unsigned i;
	AUDIOINFO info;
	AUDIOCAPS caps;
...
static video_bool sound_seal_overflow(unsigned pos, unsigned length) {
	unsigned play_pos;
...
video_error sound_seal_load(struct conf_context* context) {
	return 0;
}
