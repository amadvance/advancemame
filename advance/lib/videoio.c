...
static video_error parse_int(int* r, const char** begin, const char* end) {
	char* e;
	*r = strtol(*begin,&e,10);
	if (e==*begin || e > end) {
...
static video_error parse_token(char* token, unsigned size, const char* separator, const char** begin, const char* end) {
	unsigned pos = 0;
...
static video_error parse_quote(char* token, unsigned size, const char** begin, const char* end) {
	if (*begin == end || **begin != '"')
		return -1;
	++*begin;
...
static video_error parse_double(double* r, const char** begin, const char* end) {
	char* e;
	*r = strtod(*begin,&e);
	if (e==*begin || e > end) {
		sprintf(video_mode_parse_error,"Error in value");
...
video_error video_crtc_container_load(struct conf_context* context, video_crtc_container* cc) {
	conf_iterator i;
	int error = 0;
	...
	while (!conf_iterator_is_end(&i)) {
		...
		if (video_crtc_parse(&crtc,s, s + strlen(s)) == 0) {
			...
			if (!error) {
				error = 1;
				video_error_description_set("%s in argument '%s' for option 'device_video_modeline'",video_mode_parse_error,s);
			}
		}
...
static video_error monitor_range_parse(video_monitor_range* range, const char* begin, const char* end, double mult) {
	unsigned i = 0;
	parse_separator(" \t",&begin,end);
	while (begin < end) {
		double v0;
...
static video_error monitor_single_parse(video_monitor_range* range, const char* begin, const char* end, double mult) {
	double v0;
	double v1;
...
video_error monitor_parse(video_monitor* monitor, const char* p, const char* h, const char* v) {
	if (monitor_single_parse(&monitor->pclock,p,p+strlen(p),1E6)!=0) {
		video_error_description_set("Invalid monitor 'pclock' specification");
		return -1;
	}
	if (monitor_range_parse(monitor->hclock,h,h+strlen(h),1E3)!=0) {
		video_error_description_set("Invalid monitor 'hclock' specification");
		return -1;
	}
	if (monitor_range_parse(monitor->vclock,v,v+strlen(v),1)!=0) {
		video_error_description_set("Invalid monitor 'vclock' specification");
		return -1;
	}
	return 0;
}
...
video_error monitor_load(struct conf_context* context, video_monitor* monitor) {
	...
	if (p_present!=0) {
		video_error_description_set("Missing option 'device_video_pclock'");
		return -1;
	}
	if (monitor_single_parse(&monitor->pclock,p,p+strlen(p),1E6)!=0) {
		video_error_description_set("Invalid argument '%s' for option 'device_video_pclock'",p);
		return -1;
	}

	if (h_present!=0) {
		video_error_description_set("Missing option 'device_video_hclock'");
		return -1;
	}
	if (monitor_range_parse(monitor->hclock,h,h+strlen(h),1000)!=0) {
		video_error_description_set("Invalid argument '%s' for option 'device_video_hclock'",h);
		return -1;
	}

	if (v_present!=0) {
		video_error_description_set("Missing option 'device_video_vclock'");
		return -1;
	}
	if (monitor_range_parse(monitor->vclock,v,v+strlen(v),1)!=0) {
...
static video_error parse_generate(const char* begin, const char* end, video_generate* data) {
	double d;
	parse_separator(" \t",&begin,end);
	if (parse_double(&d,&begin,end))
		return -1;
...
static video_error parse_generate_interpolate(const char* begin, const char* end, video_generate_interpolate* interpolate) {
	double d;
...
video_error generate_parse(video_generate* generate, const char* g) {
	if (parse_generate(g,g+strlen(g),generate)!=0) {
		video_error_description_set("Invalid specification");
		return -1;
	}
...
video_error generate_interpolate_load(struct conf_context* context, video_generate_interpolate_set* interpolate) {
	...
	while (!conf_iterator_is_end(&i)) {
		...
		if (parse_generate_interpolate(s,s+strlen(s),&interpolate->map[mac])!=0) {
			video_error_description_set("Invalid argument '%s' in option 'device_video_format'",s);
			return -1;
		}
...
static video_error parse_gtf(const char* begin, const char* end, video_gtf* data) {
	double d;
	int i;
	parse_separator(" \t",&begin,end);
	if (parse_double(&d,&begin,end))
...
video_error gtf_parse(video_gtf* gtf, const char* g) {
	if (parse_gtf(g,g+strlen(g),gtf)!=0) {
		video_error_description_set("Invalid 'gtf' specification");
		return -1;
	}
...
video_error gtf_load(struct conf_context* context, video_gtf* gtf) {
	const char* s;

	if (conf_string_get(context,"device_video_gtf",&s) != 0) {
		video_error_description_set("Missing option 'device_video_gtf'");
		return 1;
	}

	if (parse_gtf(s,s+strlen(s),gtf)!=0) {
		video_error_description_set("Invalid argument '%s' for option 'device_video_gtf'",s);
		return -1;
	}
...
video_error video_snapshot_save(const char* snapshot_name, int start_x, int start_y) {
