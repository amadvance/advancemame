...
int mame_joycode(int joystick, int button)
{
	int joystick_map[8] = {
		JOYCODE_1_BUTTON1,
		JOYCODE_2_BUTTON1,
	...
	return joystick_map[joystick] + button;
}

int mame_mousecode(int mouse, int button)
{
	int mouse_map[8] = {
		MOUSECODE_1_BUTTON1,
		MOUSECODE_2_BUTTON1,
	...
	if (mouse >= 8)
		return CODE_OTHER_DIGITAL;
	if (button >= 6) /* only 6 code for mouse */
		return CODE_OTHER_DIGITAL;

	return mouse_map[mouse] + button;
}
...
void advance_input_simulate_key(struct advance_input_context* context, int key0, int key1, int frames)
{
	context->state.simulate_counter = frames;
	context->state.simulate_key0 = key0;
	context->state.simulate_key1 = key1;
}
...
static void input_keyboard_update(struct advance_input_context* context)
{
	...
	if (context->state.simulate_counter > 0) {
		if (context->state.simulate_key0 != 0 && context->state.simulate_key0 < KEYB_MAX)
			last[0][context->state.simulate_key0] = 1;
		if (context->state.simulate_key1 != 0 && context->state.simulate_key1 < KEYB_MAX)
			last[0][context->state.simulate_key1] = 1;
	...
	for (i = 0; i < INPUT_MOUSE_MAX; ++i) {
		for (j = 0; j < MOUSEB_MAX; ++j) {
			if (mac + 1 < INPUT_DIGITAL_MAX) {
				...
				/* default mapping has to use codes where available */
				/* because they are the ones that gets triggered */
				if (i < mouseb_count_get()) {
					if (j < mouseb_button_count_get(i)) {
						/* if it's an ordinal, map only if there is *no* binding */
						int k = mouseb_revbind(i, j);
						if (k < 0)
							input_code_map[mac].inputcode = mame_mousecode(i, j);
					} else {
						/* if it's a code, map only if there is a binding */
						int k = mouseb_bind(i, j);
						if (k >= 0)
							input_code_map[mac].inputcode = mame_mousecode(i, k);
					}
				}
	...
	for (i = 0; i < INPUT_JOY_MAX; ++i) {
		...
		for (j = 0; j < JOYB_MAX; ++j) {
			if (mac + 1 < INPUT_DIGITAL_MAX) {
				...
				/* default mapping has to use codes where available */
				/* because they are the ones that gets triggered */
				if (i < joystickb_count_get()) {
					if (j < joystickb_button_count_get(i)) {
						/* if it's an ordinal, map only if there is *no* binding */
						int k = joystickb_revbind(i, j);
						if (k < 0)
							input_code_map[mac].inputcode = mame_joycode(i, j);
					} else {
						/* if it's a code, map only if there is a binding */
						int k = joystickb_bind(i, j);
						if (k >= 0)
							input_code_map[mac].inputcode = mame_joycode(i, k);
					}
				}
				++mac;
			} else {
