...
static struct fileio_item CONFIG[] = {
	{ OSD_FILETYPE_ROM, "dir_rom", "rom", 0, FILEIO_MODE_COLLECTION, 0, 0 },
	{ OSD_FILETYPE_SAMPLE, "dir_sample", "sample", ".wav", FILEIO_MODE_COLLECTION, 0, 0 },
	{ OSD_FILETYPE_NVRAM, "dir_nvram" , "nvram", ".nv", FILEIO_MODE_DIRGAME, 0, 0 },
	{ OSD_FILETYPE_HIGHSCORE, "dir_hi", "hi", ".hi", FILEIO_MODE_DIRGAME, 0, 0 },
	{ OSD_FILETYPE_CONFIG, "dir_cfg", "cfg", ".cfg", FILEIO_MODE_DIRGAME, 0, 0 },
	{ OSD_FILETYPE_INPUTLOG, "dir_inp", "inp", ".inp", FILEIO_MODE_DIRGAME, 0, 0 },
	{ OSD_FILETYPE_STATE, "dir_sta", "sta", ".sta", FILEIO_MODE_DIRGAME, 0, 0 },
	{ OSD_FILETYPE_ARTWORK, "dir_artwork", "artwork", ".png", FILEIO_MODE_COLLECTION, 0, 0 },
	{ OSD_FILETYPE_MEMCARD, "dir_memcard", "memcard", ".mem", FILEIO_MODE_DIRNAME, 0, 0 },
	{ OSD_FILETYPE_SCREENSHOT, "dir_snap", "snap", ".png", FILEIO_MODE_DIRNAME, 0, 0 },
	{ OSD_FILETYPE_HIGHSCORE_DB, 0, 0, 0, FILEIO_MODE_NAME, 0, 0 }, /* used for hiscore.dat */
	{ OSD_FILETYPE_HISTORY, 0, 0, 0, FILEIO_MODE_NAME, 0, 0 }, /* used for history.dat and mameinfo.dat */
	{ OSD_FILETYPE_CHEAT, 0, 0, 0, FILEIO_MODE_NAME, 0, 0 }, /* used for cheat.dat and safequit.dat */
	{ OSD_FILETYPE_LANGUAGE, 0, 0, 0, FILEIO_MODE_NAME, 0, 0 }, /* used for language file */
#ifdef MESS
	{ OSD_FILETYPE_IMAGE, "dir_image", "image", 0, FILEIO_MODE_COLLECTION, 0, 0 },
#endif
...
static int item_has_ext(const char* file) {
	const char* slash = strrchr(file,os_dir_slash());
...
static int item_open_zip(const char* zip_file, const char* file, const char* ext, struct fileio_handle* h) {
	...
	os_log(("osd: osd_fopen() -> %s/%s\n",zip_file,file_complete));

	return 0;
}

static struct fileio_handle* item_open(int type, const char* game, const char* name, const char* mode) {
	int i;
	...
	if (!h->f) {
		os_log(("osd: osd_fopen() leave with failure\n"));
		free(h);
		return 0;
	}
...
{
	os_log(("osd: osd_faccess(newfilename:%s,filetype:%d)\n",newfilename,filetype));
...
{
	static const char* open_modes[] = { "rb", "wb", "r+b", "r+b", "w+b" };

	os_log(("osd: osd_fopen(game:%s, filename:%s, filetype:%d, mode:%d,%s)\n",game,filename,filetype,mode,open_modes[mode]));

	return item_open(filetype, game, filename, open_modes[mode]);
}
...
int osd_fread(void* void_h, void* buffer, int length)
{
	...
	os_log_pedantic(("osd: osd_fread(void_h, buffer, length:%d)\n",length));
...
int osd_fread_swap(void* void_h, void* buffer, int length)
{
	...
	os_log_debug(("osd: osd_fread_swap(void_h, buffer, length:%d)\n",length));
...
int osd_fwrite(void* void_h, const void* buffer, int length)
{
	...
	os_log_pedantic(("osd: osd_fwrite(void_h, buffer, length:%d)\n",length));
...
int osd_fwrite_swap(void* void_h, const void *buffer, int length)
{
	...
	os_log_debug(("osd: osd_fwrite_swap(void_h, buffer, length:%d)\n",length));
...
int osd_fread_scatter(void* void_h, void* buffer, int length, int increment)
{
	...
	os_log_debug(("osd: osd_fread_scatter(void_h, buffer, length:%d, increment:%d)\n",length,increment));
...
int osd_fseek(void* void_h, int offset, int whence)
{
	...
	os_log_debug(("osd: osd_fseek(void_h, offset:%d, whence:%d)\n",offset,whence));
...
void osd_fclose(void* void_h)
{
	...
	os_log_debug(("osd: osd_fclose(void_h)\n"));
...
int osd_fchecksum(const char* game, const char* filename, unsigned int* length, unsigned int* sum)
{
	...
	os_log_debug(("osd: osd_fchecksm(game:%s,  filename:%s, length, sum)\n",game,filename));
...
int osd_fsize(void* void_h)
{
	...
	os_log_debug(("osd: osd_fsize(void_h)\n"));
...
unsigned int osd_fcrc(void* void_h)
{
	...
	os_log_debug(("osd: osd_fcrc(void_h)\n"));
...
int osd_fgetc(void* void_h)
{
	...
	os_log_pedantic(("osd: osd_fgetc(void_h)\n"));
...
int osd_ungetc(int c, void* void_h)
{
	...
	os_log_pedantic(("osd: osd_ungetc(c:%d,void_h)\n",c));
...
char *osd_fgets(char* s, int n, void* void_h)
{
	...
	os_log_pedantic(("osd: osd_fgets(s,n,void_h)\n"));
...
int osd_feof(void* void_h)
{
	...
	os_log_pedantic(("osd: osd_feof(void_h)\n"));
...
int osd_ftell(void* void_h)
{
	...
	os_log_debug(("osd: osd_ftell(void_h)\n"));
...
int osd_display_loading_rom_message(const char *name, int current, int total)
{
	...
	os_log_debug(("osd: osd_display_loading_rom_message(name:%s,current:%d,total:%d)\n",name,current,total));
...
{
	os_log(("osd: osd_num_device()\n"));
	return 0;
}

void osd_change_device(const char* device)
{
	os_log(("osd: osd_change_devicee(device:%s)\n",device));
}

const char *osd_get_device_name(int i)
{
	os_log(("osd: osd_get_device_name(i:%d)\n",i));
	return "";
}
...
void* osd_dir_open(const char* dir, const char* mask)
{
	...
	os_log(("osd: osd_dir_open(dir:%s,mask:%s)\n",dir,mask));
...
void osd_dir_close(void* void_h)
{
	...
	os_log(("osd: osd_dir_close(void_h)\n"));
			...
			if (S_ISDIR(st.st_mode)) {
				if (namelength >= strlen(d->d_name) + 1) {
					*is_dir = 1;
					...
					os_log(("osd: osd_dir_get_entry() -> %s\n",name));
...
{
	os_log(("osd: osd_change_directory(dir:%s)\n",dir));
...
{
	static char cwd[OS_MAXPATH];

	os_log(("osd: osd_get_cdw()\n"));

	getcwd(cwd, sizeof(cwd));
	strcat(cwd, "/");

	os_log(("osd: osd_get_cwd() -> %s\n",cwd));
...
{
	os_log(("osd: osd_select_file(sel:%d,filename:%s)\n",sel,filename));
...
char* osd_basename(char* filename)
{
	...
	os_log(("osd: osd_basename(filename:%s)\n",filename));
...
int advance_fileio_init(struct conf_context* context) {
	...
	for(i=CONFIG;i->type != OSD_FILETYPE_end;++i) {
		if (i->config) {
			const char* def = 0;
			switch (i->mode) {
				case FILEIO_MODE_COLLECTION : def = os_config_dir_multidir(i->def); break;
				case FILEIO_MODE_DIRGAME : def = os_config_dir_singledir(i->def); break;
				case FILEIO_MODE_DIRNAME : def = os_config_dir_singledir(i->def); break;
				case FILEIO_MODE_NAME : def = os_config_dir_singlefile(); break;
			}
			if (def)
...
int advance_fileio_config_load(struct conf_context* context, struct mame_option* option) {
	...
	for(i=CONFIG;i->type != OSD_FILETYPE_end;++i) {
		if (i->config) {
			...
			os_log(("advance:fileio: %s %s\n", i->config, s));
			path_allocate(&i->dir_map,&i->dir_mac,s);
		} else {
			/* add the standard directories search as default */
			path_allocate(&i->dir_map,&i->dir_mac,os_config_dir_singlefile());
		}
	}
