...
/***************************************************************************/
/* HQ2x4 C implementation */

/*
 * This effect is derived from the hq4x effect made by Maxim Stepin
 */

void hq2x4_16_def(interp_uint16* restrict dst0, interp_uint16* restrict dst1, interp_uint16* restrict dst2, interp_uint16* restrict dst3, const interp_uint16* restrict src0, const interp_uint16* restrict src1, const interp_uint16* restrict src2, unsigned count, unsigned flag)
{
	unsigned i;
...
void hq2x4_32_def(interp_uint32* restrict dst0, interp_uint32* restrict dst1, interp_uint32* restrict dst2, interp_uint32* restrict dst3, const interp_uint32* restrict src0, const interp_uint32* restrict src1, const interp_uint32* restrict src2, unsigned count, unsigned flag)
{
	unsigned i;
...
void hq2x4_yuy2_def(interp_uint32* restrict dst0, interp_uint32* restrict dst1, interp_uint32* restrict dst2, interp_uint32* restrict dst3, const interp_uint32* restrict src0, const interp_uint32* restrict src1, const interp_uint32* restrict src2, unsigned count, unsigned flag)
{
	unsigned i;
	...
	for(i=0;i<count;++i) {
		...
		if (interp_yuy2_diff(c[0], c[4]))
			mask |= 1 << 0;
		if (interp_yuy2_diff(c[1], c[4]))
			mask |= 1 << 1;
		if (interp_yuy2_diff(c[2], c[4]))
			mask |= 1 << 2;
		if (interp_yuy2_diff(c[3], c[4]))
			mask |= 1 << 3;
		if (interp_yuy2_diff(c[5], c[4]))
			mask |= 1 << 4;
		if (interp_yuy2_diff(c[6], c[4]))
			mask |= 1 << 5;
		if (interp_yuy2_diff(c[7], c[4]))
			mask |= 1 << 6;
