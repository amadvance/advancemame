...
Configuration
  ...
  AdvanceMAME
	...
	In Windows and DOS it is the rom/ subdirectory in the current directory.

	If you want to run it with the standard video support, like any
  ...
  AdvanceMENU
	...
	and configures itself automatically. In Linux, and Mac OS X the
	...
	In Windows and DOS it is the snap/ subdirectory in the current directory.
...
Video Hardware
	...
	VGA signal, for other monitors you should take into account
  ...
  LCD monitors or LCD TVs
	...
	configuration options, and let the program use the default
	video mode.
  ...
  Multi Sync monitors (or CRT PC monitors)
	...
	and require a standard VGA connector. They are the normal PC
	monitors.
	...
	the clocks supported to allow direct video mode generation
  ...
  Fixed Sync Monitors
	...
	Generally they require separate 3, 4 or 5 BNC connectors, with
	the exception of old VGA monitors which require a standard
	VGA connector.
  ...
  Arcade Monitors
	...
	for an Extended Resolution 16 kHz are:
	...
	using a sync conversion circuit you can also try twisting
	the two H and V VGA signals together. It works if you select
	VGA negative H and V sync in the programs. To be on the safe side
  ...
  CRT TVs
	CRT TVs generally support only two fixed combinations of
	horizontal and vertical clocks, corresponding to the TV PAL
	and NTSC standards.

	Clock values for PAL TV (European) are:
	...
	for PAL TV (European) which also supports NTSC TV (USA) modes
	...
	The SCART input also accepts a composite video signal, but
	the RGB video is far superior.

	Remember that to enable the SCART RGB signal you must set
    ...
    TVs with S-Video
	If your TV has an S-Video input, you can use the TV-Out signal
	of your VGA board.

	The quality of the S-Video signal is near to the quality of
	the RGB signal.
...
Video Setup
  ...
  Operation Modes
	...
	are documented in the `advdev.txt' file.
		...
		previous setting.
		Returned to the video mode list, if the mode is
